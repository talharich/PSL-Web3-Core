...
import { usePageTransition } from '../pages/PageTransitionContext'; // NEW: Import transition context
...
export default function NFTCard({ nft, showBuy = false, onBuy }) {
  const navigate       = useNavigate();
  const prevTierRef    = useRef(nft.tier);
  const videoRef       = useRef(null);

  // NEW: Get transition context to notify on card click
  const { onCardClick } = usePageTransition();
  ...
  const grad        = TIER_GRADIENTS[nft.tier] || TIER_GRADIENTS.COMMON;
  ...
  // NEW: Enhanced click handler that notifies transition context
  const handleCardClick = () => {
    onCardClick(nft.tokenId); // Signal transition system
    navigate(`/nft/${nft.tokenId}`); // Navigate after context is updated
  };

  return (
    <div
      onClick={handleCardClick} // UPDATED: Use enhanced handler
      ...
      <div className="px-4 pb-4 flex items-center justify-between">
        ...
        {showBuy && nft.listed ? (
          <button
            ...
            className="px-4 py-1.5 text-xs rounded-xl font-semibold bg-green-500 text-black hover:scale-105 hover:shadow-[0_0_16px_rgba(34,197,94,0.4)] transition-all duration-300"
          >
